  ...
  async call(obj, number = 0) {
    ...
    if (that.cloud == null) {
      that.cloud = new wx.cloud.Cloud({
        resourceAppid: "wxae7c0ab32506ca58", // 微信云托管环境所属账号，服务商appid、公众号或小程序appid
        resourceEnv: "prod-3g053jsx715e5206", // 微信云托管的环境ID
      });
    ...
    try {
      const result = await that.cloud.callContainer({
        ...
        header: {
          "X-WX-SERVICE": "yfqsy", // xxx中填入服务名称（微信云托管 - 服务管理 - 服务列表 - 服务名称）
          // 其他 header 参数
        },
        // 其余参数同 wx.request
      });
      console.log(
        `微信云托管调用结果${result.errMsg} | callid:${result.callID}`
      );
